...
def vk_parse_get_post(group_id, post_id):
    req = f'https://api.vk.com/method/wall.getById?posts={-group_id}_{post_id}&extended' \
          f'=0&copy_history_depth=2&access_token={config.vk_token}&v=5.131'
    js_storage = requests.get(req).json()['response'][0]
    post = {}
...
def create_inline_keyboard_markup(data, lengths_rows=2):
    buttons = [InlineKeyboardButton(text=i[0], callback_data=i[1]) for i in data]
    # print(buttons)
    keyboard = InlineKeyboardMarkup(row_width=2)
    bn_n = 0
    if type(lengths_rows) == list:
...
def flow_check_new_posts(dp, bot, conn, cur, downtime):
    while config.auto_parsing_flag:
        for i in range(config.time_clean_files):
            ...
            for group_id in groups_users.keys():
                ...
                for post_num in range(len(new_posts)):
                    for user_id in groups_users[group_id]:
                        dp.loop.create_task(
                            send_new_post(bot, user_id, new_posts[post_num], group_id ))
                if len(new_posts) > 0:
...
async def send_new_post(bot, chat_id, post, group_id):
    res = return_media_message(post, group_id)
    if len(res[0].media) > 0:
        await bot.send_media_group(chat_id, res[0])
    elif res[1]:
        await bot.send_message(chat_id, res[1][0])
...
def return_new_posts(group_id, last_post_id, count_posts_check=15):
    ...
    for i in range(len(posts)):
        ...
        new_post_count += 1
    return return_new_posts(group_id, last_post_id, count_posts_check * 2)
